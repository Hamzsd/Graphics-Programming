...
void initialise()
{
	...
	if (shaders[0] && shaders[1])
	{
		...
		exit(EXIT_FAILURE);//TODO 29 is crashing on this line, seems like it cant load shaders into program

	MVPuniform = glGetUniformLocation(program, "modelViewProjection");
	colourUniform = glGetUniformLocation(program, "colour");

	geometry* geom = createPlane(2, 2);
	object.geometry = geom;
	object.colour = glm::vec4(1.0f, 0.0f, 0.0f, 0.0f);
	//object.transform.position = glm::vec3(0.0f, 0.0f, 4.0f);
